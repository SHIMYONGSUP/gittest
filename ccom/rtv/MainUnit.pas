...
uses
  Windows, SysUtils, Classes, Dialogs, ExtCtrls, StdCtrls, Controls, Graphics, ComCtrls, IniFiles, Contnrs;
...
uses GlobalVar, hmx.constant, hmx.define, HmxClass, MainForm, HmxFunc,
  helco.serial.tcp.server.mode_14byte;
...
//    shmptr^.dbconn[U_DBCONN_AGV] := DModule.DBMS.Connected;
end;
...
begin
    ...
    gWatchdogInterval := StrToIntDef(String(ReadIni(INI_SEC_DEVICE, INI_IDN_WATCHDOG_INTERVAL, INI_COM_FILE)), 0);
...
begin
    ...
    gMaxDev := StrToIntDef(String(ReadIni(INI_SEC_DEVICE, INI_IDN_MAX_QTY, INI_COM_FILE)), 0);
...
begin
    ...
    gCaption := String(ReadIni(INI_SEC_DEVICE, INI_IDN_DEV_CAPTION, INI_COM_FILE));
...
begin
    ...
    gHideInteval := StrToIntDef(String(ReadIni(INI_SEC_DEVICE, INI_IDN_HIDE_INTERVAL, INI_COM_FILE)), 0);
...
begin
    ...
    if gIniInfo[hogino].IniType.Strings[0] = INI_TYPE
    then begin
        gThread[hogino] :=
                    THelcoSerialTcpServerMode.Create(hogino,
                                                fmMain.Handle,
                                                U_WMG_BCR,
                                                gMyIni,
                                                INI_SEC_PREFIX + IntToStr(hogino),
                                                @shmptr^.grp.rtvinfo[hogino]);
    end
    else Result := False;
...
begin
    ...
    // CAPTION
    gIniInfo[hogino].IniCaption.Text := String(ReadIni(INI_SEC_PREFIX + IntToStr(hogino), INI_IDN_CAPTION, INI_COM_FILE));
    ...
    // TYPE
    gIniInfo[hogino].IniType.Text := String(ReadIni(INI_SEC_PREFIX + IntToStr(hogino), INI_IDN_TYPE, INI_COM_FILE));
    ...
    // SLEEP
    gIniInfo[hogino].IniSleep.Text := String(ReadIni(INI_SEC_PREFIX + IntToStr(hogino), INI_IDN_SLEEP, INI_COM_FILE));
    ...
    // TIMEOUT
    gIniInfo[hogino].IniTimeOut.Text := String(ReadIni(INI_SEC_PREFIX + IntToStr(hogino), INI_IDN_TIMEOUT, INI_COM_FILE));
    ...
    // PARAMETER
    gIniInfo[hogino].IniCommValue.Text := String(ReadIni(INI_SEC_PREFIX + IntToStr(hogino), INI_IDN_PARAMETER, INI_COM_FILE));
    if gIniInfo[hogino].IniCommValue.Text = ''
    then begin
        ShowMessage('INI_PARAMETER Not Found.');
        exit;
    end;

    // DISPLAY
    gIniInfo[hogino].IniDisplay.Text := String(ReadIni(INI_SEC_PREFIX + IntToStr(hogino), INI_IDN_DISPLAY, INI_COM_FILE));
    if gIniInfo[hogino].IniDisplay.Text = ''
    then begin
        ShowMessage('INI_DISPLAY Not Found.');
        exit;
    end;

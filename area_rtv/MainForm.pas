...
uses
  Windows, Messages, HmxFunc, SysUtils, Forms, Dialogs, Controls, Menus, ComCtrls,
  ExtCtrls, StdCtrls, Classes, Graphics, IniFiles, HmxClass, IdBaseComponent,
...
type
  ...
  private
    procedure ReceiveMessage(var Msg: TMessage); message U_WMG_CTL;
...
uses MainUnit, GlobalVar, hmx.control;
...
begin
    ...
    try
        ...
        // DB 연결 상태 설정
        if SetDbConnetStat <> 0
        then raise Exception.Create('SetDbConnetStat Fails!');
        ...
        // Pos Count 설정값 가져오기
        if GetPosRange <> 0
        then raise Exception.Create('GetPosRange Fails!');

        // Pass Count 설정값 가져오기
        if GetPassCount <> 0
        then raise Exception.Create('GetPassCount Fails!');
